...
public:
    CMainParams() {
        ...
        // consensus/s6b-testing: height-840,000 fork identity (S6/b scenario).
        ...
        // infrastructure, not specific to S6/b (S1 and S5/b testing branches
        ...
        consensus.ForkNo = 1;
        consensus.ForkScenarioId = 3; // S6B
        // S6/b's four fixed-value phases plus terminal cutoff, exactly as
        // specified (analysis/Rincoin_840k_S6B_Consensus_Change_Specification.qmd
        // §"Post-Activation Maximum Subsidy" and §"Terminal-Height Derivation").
        // 234,587,500 - 840,000 = 233,747,500 is *derived*, not a designed
        // round number -- see verify_s6b_independently.py's
        // derive_terminal_height() for the exact ceiling-minus-issuance
        // computation this table's last two entries encode the result of.
        consensus.ForkSubsidyPhases = {
            {0, 400000000},          // [840,000, 2,100,000): 4 RIN
            {1260000, 200000000},    // [2,100,000, 4,200,000): 2 RIN
            {3360000, 100000000},    // [4,200,000, 6,300,000): 1 RIN
            {5460000, 60000000},     // [6,300,000, 234,587,500): 0.6 RIN
            {233747500, 0},          // [234,587,500, ∞): terminal cutoff
        };
...
public:
    CTestNetParams() {
        ...
        // consensus/s6b-testing: same test-only fork identity as mainnet (see
        // CMainParams), just past the existing testnet proto-floor bump.
        // ForkSubsidyPhases is this network's own small-scale, independently
        // derived phase table (same four-phase-plus-cutoff structure and
        // derivation method as mainnet's, not a scaled copy of mainnet's
        // heights) -- see CRegTestParams for the shared rationale.
...
public:
    explicit CRegTestParams(const ArgsManager& args) {
        ...
        consensus.ForkNo = 1;
        consensus.ForkScenarioId = 3; // S6B
        // ForkSubsidyPhases is expressed as offsets from ForkH1Height (see
        // that field's doc comment in consensus/params.h), so this same
        // table applies whether ForkH1Height is the 500,000 default above
        // or the low -forkh1height= override functional tests use --
        // no separate table needed per override, unlike if this were
        // expressed as absolute heights.
        //
        // This is NOT mainnet's real table scaled down: mainnet's terminal
        // phase alone is ~228 million blocks long (derived from a
        // 168,000,000 RIN ceiling), wildly impractical to reach by mining
        // in a functional test even at a tiny scale. Instead this is a
        // small, independently chosen schedule using the *same* structure
        // and derivation method (four fixed-value phases, then a terminal
        // cutoff derived from an issuance ceiling dividing evenly by the
        // final phase's rate) -- see
        // verification/S6B/independent_derivation_regtest.md for the exact
        // small-scale derivation this table's numbers come from. Subsidy
        // *values* are deliberately left identical to mainnet's (no reason
        // to scale amounts, only heights need to be reachable); only the
        // phase-length structure is regtest-scale.
        consensus.ForkSubsidyPhases = {
            {0, 400000000},
            {50, 200000000},
            {100, 100000000},
...
public:
    explicit CPreviewParams(const ArgsManager& /*args*/) {
        ...
        // this is the network where S6/b's fork behaviour gets exercised
        // under real (if fast) PoW conditions before anything touches
        // mainnet. Kept low (well below MWEB's activation height here) for
        // the same reason as regtest -- see the comment on
        // CRegTestParams::ForkH1Height. ForkSubsidyPhases reuses the same
        // small-scale table as regtest -- see that class's comment.
