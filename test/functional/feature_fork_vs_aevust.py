...
"""Test S5/b interop against a foreign, unrelated Rincoin implementation
...
class ForkVsAevustTest(BitcoinTestFramework):
    def set_test_params(self):
        # node0: our S5/b build. node1, node2: Aevust foreign build (node2
        # only used in the IBD-hostile-majority sub-test). node3: a fresh
        # S5/b build used as the IBD client in that same sub-test.
        self.num_nodes = 4
        self.setup_clean_chain = True
        # -forkh1height= only exists on the S5/b build (node0, node3) -- the
    ...
    def header_full_block_asymmetry(self):
        ...
        self.log.info(f"Diverging node0 (S5/b) and the Aevust build independently past H1 ({FORK_H1_HEIGHT})")
    ...
    def ibd_with_hostile_peer_majority(self):
        ...
        self.log.info("Starting IBD on a fresh S5/b node connected to 2 Aevust peers + 1 correct S5/b peer")
        ...
        self.log.info("Fresh node converged on the correct S5/b chain despite a hostile-majority peer set")
